...
import waitInViewport from '../support/action/waitInViewport';
import checkFullpageScreenshot from '../support/check/checkFullpageScreenshot';

import checkEmailForChangeAccount from '../support/lib/checkEmailForChangeAccount';
import checkDowloadedFile from '../support/lib/checkDowloadedFile';
import checkRegistrationEmailToProductSite from '../support/lib/checkRegistrationEmailToProductSite';


Then(
    /^I check the full page screenshot in the name of "([^"]*)?"$/,
    checkFullpageScreenshot
);

Then(
    /^I wait that element "([^"]*)?" is( not)* within the viewport$/,
    waitInViewport
);
...
Then(
    /^I expect that the confirmation email contains that changed account info mailID: "([^"]*)?", loginID: "([^"]*)?", name: "([^"]*)?", email: "([^"]*)?", password: "([^"]*)?"$/,
    checkEmailForChangeAccount
);

  Then(
    /^I expect that the file is downloaded filename: "([^"]*)?"$/,
    checkDowloadedFile
);


Then(
    /^I expect that the confirmation of registration email language: "([^"]*)?", name: "([^"]*)?", email: "([^"]*)?"$/,
    checkRegistrationEmailToProductSite
);
